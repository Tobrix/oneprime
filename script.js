/* ═══════════════════════════════════════════
   ONEPRIMETV — script.js (Redesign 2026)
═══════════════════════════════════════════ */
...
function hideControls(){
  // Don't hide if panel or epg is open
...
controls.addEventListener('click', (e) => {
  const isBottom = e.target.closest('#bottombar');
  const isTop    = e.target.closest('#topbar');
  if(isBottom || isTop) return;

  // Close any open menu first
  if(!qualMenu.classList.contains('hidden')){
    qualMenu.classList.add('hidden'); return;
  }
  // Toggle play/pause
  video.paused ? video.play() : video.pause();
}, true);

// Touch zones for double-tap skip
...
function flashCenter(icon, stay=false){
  ...
  if(!stay){
    setTimeout(()=>indCenter.classList.remove('animating'), 250);
  }
}
...
btnMute.onclick = (e) => {
  e.stopPropagation();
  if(video.muted){ video.muted=false; if(video.volume===0) video.volume=.7; updateMuteIcon(); return; }
...
document.addEventListener('click', (e)=>{
  if(!e.target.closest('.qual-wrap')) qualMenu.classList.add('hidden');
});
...
function buildQualMenu(){
  ...
  const addOpt=(text,idx)=>{
    ...
    d.textContent=text;
    if(idx===userQuality) d.innerHTML+=` <i data-lucide="check" style="width:13px;height:13px;"></i>`;
    d.onclick=(e)=>{
      e.stopPropagation();
      userQuality=idx;
      hls.currentLevel=idx;hls.loadLevel=idx;
      qualLabel.textContent=text;
      buildQualMenu();
...
function getChannelTimes(){
  ...
  if(!el) return null;
  return { start:parseEPGDate(el.dataset.start), stop:parseEPGDate(el.dataset.stop) };
}
...
function updateTimeline(){
  ...
  if(!times||!times.start||!times.stop) return;
...
timeline.addEventListener('mousemove',(e)=>{
  ...
  const hoverT=new Date(start.getTime()+(stop-start)*pos);
...
function playStream(url, name, logo, channelId, startUnix=null, archiveData=null){
  ...
  if(Hls.isSupported()&&!isApple){
    ...
    hls.on(Hls.Events.ERROR,(e,d)=>{
      if(d.fatal){console.error('HLS fatal',d);loader.classList.add('hidden');}
    });
...
async function playNextProgram(){
  ...
  if(dayData?.length){
    ...
    if(next){
      ...
      playStream(el.dataset.url,el.querySelector('.ch-name').textContent,
        el.querySelector('img').src,currentChannelId,startUnix,next);
      return;
    }
  }
  // Fall back to live
...
window.addEventListener('keydown',(e)=>{
  ...
  switch(e.code){
    ...
    case 'Escape':
      closePanel(); closeEPGSheet(); break;
  }
});
...
function renderChannels(channels){
  ...
  channels.forEach(ch=>{
    ...
    el.innerHTML=`
      <i data-lucide="star" class="ch-fav${isFav?' starred':''}" data-id="${ch.id}"></i>
